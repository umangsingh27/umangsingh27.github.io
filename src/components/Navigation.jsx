...
export default function Navigation() {
  ...
  // Smart Header: Instant background detection (optimized for zero latency)
  useEffect(() => {
    ...
    const updateTheme = () => {
      ...
      const segmentWidth = headerRect.width / 40
      const newSegmentThemes = []
      const newPillThemes = { logo: false, links: false, cta: false }

      for (let i = 0; i < 40; i++) {
        const x = headerRect.left + (i + 0.5) * segmentWidth

        const samples = [
          detectThemeAtPoint(x, headerRect.top + 5),
          detectThemeAtPoint(x, headerRect.top + headerRect.height / 2),
          detectThemeAtPoint(x, headerRect.top + headerRect.height - 5)
        ]

        newSegmentThemes[i] = samples.some(s => s)
      }

      newPillThemes.logo = newSegmentThemes.slice(0, 10).some(s => s)
      newPillThemes.links = newSegmentThemes.slice(10, 30).some(s => s)
      newPillThemes.cta = newSegmentThemes.slice(30, 40).some(s => s)

      setSegmentThemes(newSegmentThemes)
      setPillThemes(newPillThemes)
    }
    ...
    // Instant update on scroll for continuous theme detection
    const handleScroll = () => {
      updateTheme()
    }
    ...
    <header className="nav-header">
      <div className="nav-header-bg">
        {segmentThemes.map((isDark, i) => (
          <div key={i} className={`nav-bg-segment ${isDark ? 'dark' : ''}`} />
        ))}
      </div>

      <div className="nav-inner">
        {/* Logo Pill */}
        <a href="/" className={`nav-logo-pill ${pillThemes.logo ? 'pill--dark' : ''}`}>
          <svg width="95" height="13" viewBox="0 0 119 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="nav-logo-image">
        ...
        {/* CTA Button */}
        <a href="mailto:hello@umangsingh.dev" className={`nav-cta-btn ${pillThemes.cta ? 'pill--dark' : ''}`}>
          Get in Touch
        </a>
      ...
      <div className={`nav-overlay ${menuOpen ? 'nav-overlay--open' : ''}`}>
        ...
        <div className="overlay-cta-container">
          <a href="mailto:hello@umangsingh.dev" className="overlay-cta" onClick={() => setMenuOpen(false)}>
            Get in Touch
          </a>
        </div>
      </div>
